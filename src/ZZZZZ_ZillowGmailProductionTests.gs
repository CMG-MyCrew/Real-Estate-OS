/** Safe production-readiness smoke test. Does not send email or create leads. */
function reosTestZillowGmailProductionReadiness() {
  var checks = [];
  function check(name, pass, details) {
    checks.push({ name: name, pass: !!pass, details: details || '' });
  }
  check('Production module loaded', !!REOS.ZillowGmailProduction);
  check('Base connector loaded', !!REOS.ZillowGmailConnector);
  ...
  try {
    var sheets = REOS.ZillowGmailProduction.ensureSheets();
    check('Production sheets ensured', !!sheets && sheets.ok === true, JSON.stringify(sheets));
  } catch (error) {
  ...
  return { ok: failed.length === 0, passed: checks.length - failed.length, failed: failed.length, checks: checks, status: statusResult };
}
