...
REOS.LivePipelineVerification = (function () {
  ...
  function start() {
    ...
    var state = {
      ...
      status: 'In Progress', stageIndex: 0, stageCount: STAGES.length,
      leadId: getId_(lead), address: ADDRESS, startedAt: started.toISOString(),
      completedAt: '', checks: [], errors: [], executedBy: currentUser_()
    };
    saveState_(state);
    audit_(state.runId, 0, 'RUN_STARTED', RUNS, state.runId, {
      leadId: state.leadId, createdLead: !!leadResult.created, stageCount: state.stageCount
    });
    return status();
  }
  ...
  function runNextStage() {
    ...
    try {
      ...
      audit_(state.runId, stage.id, 'STAGE_COMPLETED', '', '', {
        stageId: stage.id, stageName: stage.name, nextStageIndex: state.stageIndex,
  ...
  function stageIngest_(state) {
    ...
    invokeStage_(state.runId, 2, 'Ingestion orchestrator', REOS.AcquisitionIngestionOrchestrator, 'run', [{
      runConnectors: false, scanDuplicates: false, scoreLeads: false, autoPromote: false
    }]);
  ...
  function stageAcquisitionIntelligence_(state) {
    ...
    return writeResult_(state.runId, 3, 'Acquisition intelligence completed', 'IA_LEADS', 'IA_LEADS', state.leadId,
      rows.length ? getId_(rows[0]) : '', state.leadId, rows.length ? 'Pass' : 'Fail', 0,
  ...
  function stageOfferAutomation_(state) {
    ...
    return verifyStage_(state.runId, 5, 'Offer queue record', 'AI_DEAL_INTELLIGENCE', 'AI_OFFER_QUEUE', state.leadId,
  ...
  function stageOfferReview_(state) {
    ...
    return verifyStage_(state.runId, 6, 'Offer review record', 'AI_OFFER_QUEUE', 'AI_OFFER_REVIEW', state.leadId,
  ...
  function stageOfferExecution_(state) {
    ...
    var offerCheck = verifyStage_(state.runId, 7, 'Offer record', 'AI_OFFER_REVIEW', 'OFFERS', state.leadId,
      function () { return findRelatedRows_('OFFERS', state.leadId); });
    state.checks.push(offerCheck);
    return verifyStage_(state.runId, 7, 'Execution queue record', 'OFFERS', 'OFFER_EXECUTION_QUEUE', state.leadId,
      function () { return findRelatedRows_('OFFER_EXECUTION_QUEUE', state.leadId); });
  }

  function stageFinalize_(state) { return verifyDuplicates_(state.runId, 8, state.leadId); }
  ...
  function finalize_(state) {
    ...
    state.completedAt = completed.toISOString(); state.passed = passed; state.failed = failed;
    state.integrityPercent = integrity; state.duplicateProtection = duplicate ? duplicate.status : 'Not Run';
    var summary = { runId: state.runId, status: state.status, leadId: state.leadId, passed: passed, failed: failed,
      integrityPercent: integrity, duplicateProtection: state.duplicateProtection, checks: state.checks, errors: state.errors };
    REOS.Database.insert(RUNS, {
      'Run ID': state.runId, Status: state.status, 'Lead ID': state.leadId, Address: state.address,
      Passed: passed, Failed: failed, 'Integrity Percent': integrity, 'Duplicate Protection': state.duplicateProtection,
      'Started At': new Date(state.startedAt), 'Completed At': completed,
      'Duration Ms': completed.getTime() - new Date(state.startedAt).getTime(),
      'Summary JSON': JSON.stringify(summary), 'Executed By': state.executedBy || currentUser_()
  ...
  function status() {
    ...
    return {
      ok: state.status !== 'Needs Attention', active: state.status === 'In Progress', runId: state.runId,
      status: state.status, leadId: state.leadId, completedStages: Math.min(state.stageIndex, STAGES.length),
      totalStages: STAGES.length, nextStage: next ? next.name : '',
      ...
      integrityPercent: Number(state.integrityPercent || 0), duplicateProtection: state.duplicateProtection || 'Pending',
      errors: state.errors || []
    };
  }
  ...
  function invokeStage_(runId, stageId, stage, module, method, args) {
    ...
    audit_(runId, stageId, 'INVOKED_' + method.toUpperCase(), '', '', {
      stage: stage, result: safeJson_(value), durationMs: new Date().getTime() - start.getTime()
    });
    return value;
  }
  ...
  function verifyStage_(runId, stageId, stage, source, destination, parentId, finder) {
    var start = new Date();
    try {
      var rows = finder() || []; var row = rows[0] || null; var ok = !!row;
      return writeResult_(runId, stageId, stage, source, destination, parentId, row ? getId_(row) : '', parentId,
        ok ? 'Pass' : 'Fail', new Date().getTime() - start.getTime(), ok ? 'Record located' : 'No related record located');
    } catch (error) {
      return writeResult_(runId, stageId, stage, source, destination, parentId, '', parentId, 'Fail',
        new Date().getTime() - start.getTime(), error.message || String(error));
    }
  }
  ...
  function verifyDuplicates_(runId, stageId, leadId) {
    ...
    return writeResult_(runId, stageId, 'Duplicate protection', '', '', leadId, '', leadId,
      duplicates.length ? 'Fail' : 'Pass', 0, duplicates.length ? duplicates.join(', ') : 'No duplicate test records detected');
  }
  ...
  function writeResult_(runId, stageId, stage, source, destination, inputId, outputId, parentId, statusValue, duration, message) {
    var row = { stageId: Number(stageId || 0), stage: stage, status: statusValue, sourceSheet: source,
      destinationSheet: destination, inputRecordId: inputId || '', outputRecordId: outputId || '',
      parentRecordId: parentId || '', durationMs: duration || 0, message: message || '' };
    REOS.Database.insert(RESULTS, {
      'Run ID': runId, 'Stage ID': Number(stageId || 0), Stage: stage, 'Source Sheet': source,
      'Destination Sheet': destination, 'Input Record ID': row.inputRecordId, 'Output Record ID': row.outputRecordId,
      'Parent Record ID': row.parentRecordId, Status: statusValue, 'Duration Ms': row.durationMs,
  ...
  function audit_(runId, stageId, event, sheet, recordId, details) {
    REOS.Database.insert(AUDIT, {
      'Run ID': runId, 'Stage ID': Number(stageId || 0), Event: event, Sheet: sheet || '',
      'Record ID': recordId || '', 'Details JSON': JSON.stringify(details || {}), 'Created At': new Date()
  ...
  function migrateStageIdColumns_() {
    ...
    migrateSheetStageId_(ss, AUDIT, 'Event', function (record) {
      try { var details = JSON.parse(record['Details JSON'] || '{}'); return Number(details.stageId || 0); }
      catch (error) { return 0; }
    });
    migrateSheetStageId_(ss, RESULTS, 'Stage', function (record) {
      var stageIds = {
        'Distress lead': 1, 'Intelligent acquisition lead': 2, 'Acquisition intelligence completed': 3,
        'Deal intelligence record': 4, 'Offer queue record': 5, 'Offer review record': 6,
        'Offer record': 7, 'Execution queue record': 7, 'Duplicate protection': 8
      };
      return Number(stageIds[record.Stage] || 0);
    });
  }
  ...
  function saveState_(state) { PropertiesService.getScriptProperties().setProperty(STATE_KEY, JSON.stringify(state)); }
  function getId_(record) {
    if (!record) return '';
    var keys = Object.keys(record);
    for (var i = 0; i < keys.length; i++) if (/\bID$/i.test(keys[i]) && record[keys[i]]) return String(record[keys[i]]);
    return '';
  }
  function normalize_(value) { return String(value || '').toLowerCase().replace(/[^a-z0-9]/g, ''); }
  function safeJson_(value) { try { return JSON.parse(JSON.stringify(value)); } catch (error) { return String(value); } }
  ...
  return {
    ensureSheets: ensureSheets, createTestLead: createTestLead, start: start, run: run,
    runNextStage: runNextStage, status: status, finalize: finalize, reset: reset, summary: summary
  };
})();
